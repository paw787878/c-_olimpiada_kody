...
void djakira(vector<vector<pair<int, pair<int,int>>>> &graf, vector<long long int> &odleglosci)
{
    long long int nies = numeric_limits<long long int>::max() / 10;
    ...
    while (!heap_odleglosci.empty())
    {
        ...
        for (int i = 0; i < graf[jaki_wierz].size(); i++)
        {
            ...
            long long int waga_krawedzi = graf[jaki_wierz][i].second.first;
            if (!czy_usuniety[id_syna])
                heap_odleglosci.minimalize(wskazniki[id_syna], min(wskazniki[id_syna]->war, jaka_wartosc + (long long int)waga_krawedzi));
...
void dorob_wirtualne(vector<vector<pair<int, pair<int,int>>>> &graf, int &wyjscie)
{
    int m = graf.size();
    wyjscie = m;
    ...
    {
        pair<int,pair<int,int>> para = graf[0][i];
        int sasiad = para.first;
        int wirtualny = graf.size();
        vector<pair<int, pair<int,int>>> pom;
        graf.push_back(pom);
        int koszt_zejscia=graf[0][i].second.second;
        graf[wirtualny].push_back(pair<int, pair<int,int>>(wyjscie, pair<int,int>(koszt_zejscia,-1)));
        for (int j = 0; j < graf[sasiad].size(); j++)
            if (graf[sasiad][j].first != 0&& graf[sasiad][j].first<wyjscie)
            {
                //graf[wirtualny].push_back(graf[sasiad][j]);//bo nie trzeba tego robic
                int som_somsiad=graf[sasiad][j].first;
                int jaka_waga=graf[sasiad][j].second.second;
                graf[som_somsiad].push_back(pair<int,pair<int,int> >(wirtualny,pair<int,int>(jaka_waga,-1)));
            }
    }
}

void wypisz_graf(vector<vector<pair<int,pair<int,int>>>>& graf){
    for(int i=0;i<graf.size();i++){
        cout<<endl<<"wypisuje "<<i+1<<" ";
        for(int j=0;j<graf[i].size();j++){
            cout<<"( "<<graf[i][j].first+1<<", "<<graf[i][j].second.first<<" ) ";
        }
    }
}
...
int main()
{
    ...
    for (int i = 0; i < m; i++)
    {
        int a, b, c, d;
        we >> a >> b >> c >> d;
        graf[a - 1].push_back(pair<int, pair<int,int>>(b - 1, pair<int,int>(c,d)));
        graf[b - 1].push_back(pair<int, pair<int,int>>(a - 1, pair<int,int>(d,c)));
    }
    //teraz dorabiamy wirtualnych gosci oraz wyjscie
    int wyjscie;
    dorob_wirtualne(graf, wyjscie);
    //cout<<"wyjscie to "<<wyjscie+1;
    //wypisz_graf(graf);
    vector<long long int> odleglosci;
    djakira(graf,odleglosci);
    cout<<odleglosci[wyjscie];

    /*
    //ifstream we("dane.txt");
    int n;
    cin>>n;
    vector<long long int> polowy_cen;
    for(int i=0;i<n;i++){
        int pom;
        cin>>pom;
